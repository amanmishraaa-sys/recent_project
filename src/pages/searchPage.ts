...
export class SearchPage{
    ...
    constructor(readonly page: Page){
        ...
        this.locationSelection = (fieldName: string) => page.getByRole('combobox',{ name: fieldName });
        ...
        this.oneWaySelect = page.locator('[class="Popup__content"] div span').getByText('One-way');
        this.firstSelection = page.locator('li div p span').getByText('Indira Gandhi International Airport');
        this.secondSelection = page.locator('li div p span').getByText('Chhatrapati Shivaji Maharaj International Airport');
        this.selectDate = (dateText: string) => page.getByRole('button',{ name: dateText });
        this.searchForHotelCheckbox = page.locator('label').filter({ hasText: "Search for hotel in different cities or dates"}).locator('span span');
        this.searchButton = page.getByRole('button',{ name: "SEARCH FLIGHT + HOTEL" });
        this.errorMessage = page.locator('[class="ModalMessage ModalMessage--center"]').getByText('Please enter the name of a country, city, airport, neighborhood, landmark, or property to proceed.');
    }
    ...
    async fillLocationForFlyingFromField(){
        await this.handleAutoSuggestionBox(this.locationSelection("Flying from"), "New Delhi (DEL)", this.firstSelection);
    }

    async fillLocationForFlyingToField(){
        await this.handleAutoSuggestionBox(this.locationSelection("Flying to"), "Mumbai (BOM)", this.secondSelection);
    }
    ...
    async selectDepartureDate(day: string){
        const dateText: string = day.toLowerCase() == "today"? this.getTodaysDate(): this.getTomorrowsDate();
